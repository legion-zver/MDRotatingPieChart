...
@objc protocol MDRotatingPieChartDelegate {
    ...
    /**
     ...
     :param: index slice index in your data array
     */
    @objc optional func willOpenSliceAtIndex(index:Int)
    
    /**
     Triggered when a slice is going to be closed
     :param: index slice index in your data array
     */
    @objc optional func willCloseSliceAtIndex(index:Int)
    
    /**
     Triggered when a slice has just finished opening
     :param: index slice index in your data array
     */
    @objc optional func didOpenSliceAtIndex(index:Int)
    
    /**
     Triggered when a slice has just finished closing
     :param: index slice index in your data array
     */
    @objc optional func didCloseSliceAtIndex(index:Int)
}
...
class MDRotatingPieChart: UIControl {
    ...
    func build() {
        ...
        if(datasource == nil) {
            NSLog("Did you forget to set your datasource ?")
            return
        }
     ...
     */
    func closeSlice() {
        delegate?.willCloseSliceAtIndex!(index: currentSelected)
        slicesArray[currentSelected].shapeLayer.transform = oldTransform!
        delegate?.didCloseSliceAtIndex!(index: currentSelected)
        labelCenter.text = ""
    }
    ...
    func openSlice(index:Int) {
        ...
        delegate?.willOpenSliceAtIndex!(index: index)
        slicesArray[index].shapeLayer.transform = translate
        
        delegate?.didOpenSliceAtIndex!(index: index)
    ...
    func createSlice(start:CGFloat, end:CGFloat, color:UIColor, label:String, value:CGFloat, percent:CGFloat) -> Slice {
        ...
        return Slice(myPaths: path, myShapeLayer: mask, myAngle: end-start, myLabel:label, myValue:value, myPercent:percent);
    }
    ...
    func computeDualPath(start:CGFloat, end:CGFloat) -> DualPath {
        ...
        let other = pathRef.cgPath.copy(strokingWithWidth: properties.bigRadius-properties.smallRadius, lineCap: .butt, lineJoin: .miter, miterLimit: 1)
        
        return DualPath(myBezierPath: UIBezierPath(cgPath: other), myAnimationBezierPath: pathRef)
    }
